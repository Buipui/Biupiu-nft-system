# Biupiu World — New Age Theory Character & Avatar System v1.0

**Status:** Character/world-building specification  
**Purpose:** Give the New Age Theory gateway recognisable non-player characters, guides, researchers and player-avatar archetypes while keeping historical people, folklore and fictional representations clearly separated.

## Entry-point characters

### NAT-GUIDE-01 — The Archivist
Role: Main Hub / New Age Theory gateway guide.
Visual language: premium futuristic research attire, archive tablet, layered map/globe interface.
Function: explains evidence states, source provenance and navigation.
Classification: FICTIONAL BIOUPIU WORLD CHARACTER.

### NAT-GUIDE-02 — The Cartographer
Role: Ancient-map and lost-land research guide.
Visual language: explorer/researcher clothing, historical maps, GIS overlays and transparent terrain models.
Function: routes users into Tartaria, Thule, Hy-Brasil, Atlantis and submerged-land research.
Classification: FICTIONAL BIOUPIU WORLD CHARACTER.

### NAT-GUIDE-03 — The Geophysicist
Role: Hollow Earth / geology reality-check guide.
Visual language: modern field-science clothing, seismic/geophysical instrumentation and Earth cross-section interface.
Function: contrasts speculative inner-Earth models with documented geophysics.
Classification: FICTIONAL BIOUPIU WORLD CHARACTER.

### NAT-GUIDE-04 — The Archaeotechnology Researcher
Role: Ancient technology claim-testing guide.
Visual language: field archaeologist + engineer, measurement tools, material samples and reconstruction interface.
Function: sends claims to archaeology, materials, metallurgy, geometry and experimental laboratories.
Classification: FICTIONAL BIOUPIU WORLD CHARACTER.

## Environment avatars

### Atlantis
- Atlantean Scholar — fictional reconstruction
- Maritime Navigator — fictional reconstruction
- Architecture Researcher — fictional reconstruction

### Mu
- Pacific Cartographer — fictional research avatar
- Ocean-Geology Researcher — fictional research avatar
- Materials/Crystal Researcher — fictional research avatar

### Hyperborea
- Northern Mythology Archivist — fictional research avatar
- Polar Cartographer — fictional research avatar
- Classical Source Researcher — fictional research avatar

### Tartaria
- Historical Cartographer — fictional research avatar
- Central Asian Historical Researcher — fictional research avatar
- Great Tartaria Claim Analyst — fictional research avatar

### Hollow Earth
- Geophysics Researcher — fictional research avatar
- Inner-Earth Folklore Archivist — fictional research avatar
- Seismic Data Analyst — fictional research avatar

### Agartha / Shambhala
- Himalayan Cultural Researcher — fictional research avatar
- Textual Traditions Archivist — fictional research avatar
- Comparative Mythology Researcher — fictional research avatar

### Lost Lands / Cities
- Submerged Landscape Archaeologist
- Remote-Sensing Specialist
- Desert Exploration Historian
- Maritime Archaeology Researcher
- Cultural Memory Researcher

## Player character archetypes

Players may select a neutral research avatar rather than a historical/legendary person:

1. Researcher
2. Archaeologist
3. Materials Scientist
4. Metallurgist
5. Geologist
6. GIS / Cartography Specialist
7. Textile Researcher
8. Engineer
9. Marine Researcher
10. Historian / Source Critic
11. Experimentalist
12. Archivist

## Character-generation constraints

- Characters must not be presented as actual historical inhabitants unless independently documented and explicitly identified.
- Legendary characters/entities may be represented as folklore-inspired visualisations, not documentary portraits.
- No invented quotation may be attributed to a historical or religious figure.
- Clothing, architecture, tools and environments are labelled as reconstructed, inspired or fictional where evidence is incomplete.
- Characters inherit the user's subscriber department permissions.
- A character cannot bypass evidence or department access controls.
- AI-generated character dialogue must cite the underlying source/research object when making factual historical claims.
- Speculative dialogue must be marked as hypothesis, tradition, folklore or fiction as appropriate.

## Main Hub interaction

NEW AGE THEORY GATEWAY
→ Archivist introduction
→ choose Lost Civilisations / Lost Lands / Inner Earth / Ancient Maps / Ancient Technology
→ select environment
→ meet environment guide
→ enter evidence room
→ enter department workshop
→ create research project

## Asset registry

Character assets use:
CHARACTER ID → ENVIRONMENT ID → ROLE → VISUAL PROVENANCE → EVIDENCE STATE → DEPARTMENT ACCESS → ASSET VERSION.

All 3D models, concept art, animations, dialogue packs and costumes remain versioned repository assets.

## First production batch

The initial character pack should prioritise:
- The Archivist
- The Cartographer
- The Geophysicist
- The Archaeotechnology Researcher
- Atlantis Scholar
- Mu Cartographer
- Hyperborean Archivist
- Tartaria Cartographer
- Hollow Earth Geophysicist
- Submerged Landscape Archaeologist

These are original Biupiu World characters rather than depictions of specific living or historical people.


## Visual-reference expansion — video/descriptive research layer

Character visual design is now allowed to draw from **archived video footage, descriptive narratives and visual references associated with each speculative tradition**, while keeping a strict provenance distinction between source material and established history.

### Reference interpretation

The research corpus shows several recurring visual conventions:

- **Atlantis:** monumental maritime architecture, coastal/underwater settings, advanced-looking geometric structures and seafaring imagery in modern speculative media.
- **Mu:** Pacific/island landscapes, monumental ruins, tropical environments, crystal/mineral motifs and oceanic settings in modern speculative interpretations.
- **Hyperborea:** polar/northern landscapes, solar/Apollo symbolism, monumental or archaic architecture and esoteric “ancient northern people” imagery. Some modern fictional treatments add highly fantastical humanoid traits; those are tagged CREATIVE/SPECULATIVE rather than historical.
- **Tartaria:** ornate monumental civic architecture, domes, towers, star forts, grand interiors, old-world clothing and “pre-reset” urban imagery frequently used in online theory videos. Some sources portray alleged inhabitants as giants; that is a claim-specific visual variant, not a factual biological conclusion. Recent online video descriptions explicitly frame many visualisations as dramatized or speculative. citeturn0search0turn0search5turn0search7
- **Hollow Earth / Agartha:** subterranean cities, immense caverns, inner suns/lighting systems, crystalline environments and expedition aesthetics in esoteric/fictional representations.
- **Lost-land environments generally:** use source-specific architecture, terrain, clothing, tools and materials where available, with fictional interpolation only in clearly marked layers.

### Character-model asset pipeline

Existing **allocated mod assets** may be reused as source/reference assets only where the repository records their licence/permission and provenance.

Asset pipeline:

MOD ASSET / VIDEO REFERENCE
→ LICENCE CHECK
→ ASSET ID
→ SOURCE / VIDEO ID
→ ENVIRONMENT ID
→ VISUAL FEATURE EXTRACTION
→ CHARACTER / ENVIRONMENT VARIANT
→ EVIDENCE STATE
→ RIGHTS / LICENCE TAG
→ OPTIMISED GAME ASSET
→ WORLD INTEGRATION

### Mod-asset rules

1. Do not assume a mod asset is legally reusable merely because it is downloadable.
2. Record the original mod, creator, licence, source URL, asset filename and permitted use.
3. Separate **Biupiu-original assets**, **licensed/permissioned mod assets**, **public-domain assets** and **reference-only assets**.
4. If a mod has a restrictive licence, the asset remains reference-only unless permission is obtained.
5. Do not redistribute extracted copyrighted game assets as part of a public Biupiu package unless the licence permits it.
6. The final character system should support swappable meshes, textures, clothing, accessories and animation rigs so licensed replacements can be substituted without changing world logic.

## Environment visual-design matrix

### Atlantis
Visual layers:
- coastal/underwater metropolis
- monumental stone/metal architecture
- submerged ruins
- marine research zone
- speculative energy/crystal zone
- navigation/shipyard area

### Mu
Visual layers:
- tropical Pacific island geography
- oceanic settlements
- monumental terraces
- botanical zones
- mineral/crystal research spaces
- submerged/coastal ruins

### Hyperborea
Visual layers:
- northern/polar terrain
- ice/snow and highland zones
- monumental ceremonial architecture
- astronomical/solar symbolism
- archive and mythology spaces
- speculative advanced-technology layer

### Tartaria
Visual layers:
- ornate imperial/civic architecture
- domed halls and towers
- star-fort geometry
- monumental railway/exhibition/civic spaces
- historical-map district
- “reset/mud-flood” claim reconstruction district
- architecture-forensics laboratory

### Hollow Earth
Visual layers:
- deep cavern entrances
- enormous subterranean chambers
- underground rivers
- geothermal/mineral zones
- speculative inner-city environments
- geophysics reality-check laboratory

## Visual evidence states

Each visual component receives:
- DOCUMENTED REFERENCE
- HISTORICAL RECONSTRUCTION
- SOURCE-BASED SPECULATION
- CREATIVE INTERPRETATION
- GAME/WORLD-BUILDING FICTION
- REFERENCE ONLY

This prevents a video-game mod, AI image, YouTube reconstruction or speculative artwork from being mistaken for archaeological evidence.

## Production priority

The first environment/character bundle should use the allocated assets as **reference/implementation candidates** for:

1. Tartaria — character + grand civic environment
2. Atlantis — maritime/underwater character + environment
3. Hyperborea — northern/polar character + environment
4. Mu — Pacific/oceanic character + environment
5. Hollow Earth — subterranean character + environment

The Biupiu World engine should expose these as interchangeable environment packages rather than hard-coding one visual interpretation as historical truth.
