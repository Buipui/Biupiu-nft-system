...
impl BiupiuSimulationKernel {
    pub fn new(simulation_id: impl Into<String>) -> Self {
        Self { simulation_id: simulation_id.into(), ..Default::default() }
    }
    ...
    pub fn record_measurement(&mut self, entity_id: &str, metric: impl Into<String>, value: f64) -> Result<(), String> {
        ...
        self.measurements.push(SimulationMeasurement {
...
impl Plugin for BiupiuBevyPlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<BiupiuBevyRuntime>()
            .init_resource::<BiupiuSimulationKernel>()
...
mod tests {
    ...
    #[test]
    fn plugin_registers_runtime_and_kernel() {
        let mut app = App::new();
    ...
    #[test]
    fn kernel_rejects_duplicate_and_unknown_entities() {
        let mut kernel = BiupiuSimulationKernel::new("SIM-001");
        let entity = SimulationEntity { id: "E-001".into(), kind: "test".into(), state: BTreeMap::new() };
        assert!(kernel.register_entity(entity.clone()).is_ok());
        assert!(kernel.register_entity(entity).is_err());
        assert!(kernel.record_measurement("UNKNOWN", "value", 1.0).is_err());
    }
    #[test]
    fn kernel_advances_only_when_running_and_records_measurements() {
        let mut kernel = BiupiuSimulationKernel::new("SIM-001");
        kernel.register_entity(SimulationEntity { id: "E-001".into(), kind: "test".into(), state: BTreeMap::new() }).unwrap();
        kernel.step();
        assert_eq!(kernel.tick, 0);
        kernel.start();
        kernel.step();
        assert_eq!(kernel.tick, 1);
        kernel.record_measurement("E-001", "velocity", 12.5).unwrap();
        assert_eq!(kernel.measurements[0].tick, 1);
    }
    #[test]
    fn kernel_replay_fixture_is_deterministic() {
        fn run() -> Vec<(u64, f64)> {
            ...
            for value in [1.0, 2.0, 3.0] {
                k.step();
                k.record_measurement("E-001", "value", value).unwrap();
            }
