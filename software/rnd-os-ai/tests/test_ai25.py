...
def test_ai25_provenance_accepts_registered_resource():
    assert validate_resource_provenance(["NASA-FPRIME"], Registry()).accepted

def test_ai25_provenance_rejects_unknown_resource():
    assert not validate_resource_provenance(["UNKNOWN"], Registry()).accepted

def test_ai25_transactional_audit_boundary_deduplicates_request_event():
    boundary = TransactionalAuditBoundary(InMemoryAuditStore())
    event = DurableAuditEvent("1.0", "req-25", "gateway.accepted", "success", "client")
