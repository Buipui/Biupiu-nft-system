...
counts are never hard-coded. Safety-critical execution remains behind a
...
class ComputeFederation:
    ...
    def eligible_units(self, workload: Workload) -> list[ComputeUnit]:
        units = [u for u in self.topology.units if u.available and u.capacity > 0
    ...
    def plan(self, workloads: Iterable[Workload]) -> dict[str, str]:
        ...
        for work in sorted(workloads, key=lambda w: (-w.cost, w.workload_id)):
            ...
            loads[unit.unit_id] += work.cost if work.parallelizable else work.cost * 1.25
        return plan

    def execute(self, workload: Workload, runner: Callable[[Workload, ComputeUnit], object]) -> object:
