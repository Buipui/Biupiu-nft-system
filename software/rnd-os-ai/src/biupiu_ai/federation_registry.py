...
def eligible_for_activation(system: AgentSystem, policy: FederationPolicy, *, provenance=False, license=False, security=False, regression=False, human_approved=False) -> bool:
    if system.status == "BLOCKED":
        return False
    checks = (provenance, license, security, regression, human_approved)
    required = (policy.require_provenance, policy.require_license, policy.require_security, policy.require_regression, policy.require_human_promotion)
