...
const { validateResearchObject, EVIDENCE_STATES, MATURITY } = require('./lib/domain');
...
const server = http.createServer(async (req,res)=>{
  try {
    ...
    if(req.method==='GET' && url.pathname==='/api/health') return json(res,200,{ok:true,service:'Biupiu R&D OS',version:'0.1.1'});
    ...
    if(m && req.method==='POST') { const d=db(), c=m[1], body=await readBody(req); if(c==='research'){ const errors=validateResearchObject(body); if(errors.length) return json(res,400,{errors}); } const record={id:id(collections[c]),createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),...body}; d[c].push(record); audit(d,'CREATE',c,record.id); save(d); return json(res,201,record); }
