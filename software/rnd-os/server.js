...
function db(){return readDb()} function save(x){return writeDb(x)}
...
function tenantPackage(d,user){const out={...d};for(const c of ['projects','research','hypotheses','experiments','failures','ip','relationships','evidence'])out[c]=scopedRecords(d,user,c);out.audit=(d.audit||[]).filter(x=>!x.payload?.organisationId||x.payload.organisationId===user.organisationId);out.sessions=[];out.users=[];out.organisations=[];return out}
const collections={projects:'PRJ',research:'RES',hypotheses:'HYP',experiments:'EXP',failures:'FAIL',ip:'IP'};

const server=http.createServer(async(req,res)=>{
 let principal=null,activeDb=null;
 try{
  ...
  if(req.method==='GET'&&url.pathname==='/api/package/export')return json(res,200,exportPackage(tenantPackage(activeDb,principal)));
  ...
  if(activeDb){recordSecurityEvent(activeDb,{method:req.method,path:req.url,userId:principal?.id||null,organisationId:principal?.organisationId||null,outcome:status>=400?'denied':'error'});writeDb(activeDb)}
