...
try {
  ...
  switch($Operation){
    'HOST.CAPABILITIES' { $graphics=$null; if($IncludeGraphics){$graphics=[ordered]@{gpu=@(Get-CimInstance Win32_VideoController -ErrorAction SilentlyContinue | Select-Object Name,AdapterCompatibility,DriverVersion,AdapterRAM,VideoProcessor,CurrentHorizontalResolution,CurrentVerticalResolution,CurrentRefreshRate);executables=@{cmake=(Get-Command cmake -ErrorAction SilentlyContinue).Source;git=(Get-Command git -ErrorAction SilentlyContinue).Source;dotnet=(Get-Command dotnet -ErrorAction SilentlyContinue).Source;cargo=(Get-Command cargo -ErrorAction SilentlyContinue).Source};os=(Get-CimInstance Win32_OperatingSystem -ErrorAction SilentlyContinue | Select-Object Caption,Version,BuildNumber,OSArchitecture)}; New-Evidence 'EXECUTED' @{os=[Environment]::OSVersion.VersionString;computer=$env:COMPUTERNAME;architecture=$env:PROCESSOR_ARCHITECTURE;execution_policy=(Get-ExecutionPolicy -Scope Process);execution_policy_list=(Get-ExecutionPolicy -List | ForEach-Object { [pscustomobject]@{scope=$_.Scope;policy=$_.ExecutionPolicy} });graphics=$graphics} | ConvertTo-Json -Depth 10 }
