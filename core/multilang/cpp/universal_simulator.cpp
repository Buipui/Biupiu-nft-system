...
static bool biupiu_finite(double v) { return std::isfinite(v); }
...
extern "C" int biupiu_sim_validate_context(const BiupiuSimContext* ctx) {
    ...
    if (!biupiu_finite(ctx->time_s) || !biupiu_finite(ctx->dt_s) || ctx->dt_s <= 0.0) return -3;
    return 0;
}
...
extern "C" int biupiu_sim_record_observation(const BiupiuSimContext* ctx,
    ...
    if (!biupiu_finite(input_hash64) || !biupiu_finite(output_hash64) || !biupiu_finite(residual) ||
        !biupiu_finite(confidence) || confidence < 0.0 || confidence > 1.0) return -2;
