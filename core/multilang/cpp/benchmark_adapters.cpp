...
extern "C" int biupiu_benchmark_validate(const BiupiuBenchmarkModule* m){
 if(!m||!nonempty(m->capability)||!nonempty(m->native_target)||!nonempty(m->evidence_state)||!nonempty(m->provenance_ref)) return -1;
...
extern "C" int biupiu_benchmark_classify(BiupiuBenchmarkId id,BiupiuHarvestClass c,BiupiuBenchmarkModule* out){
 ...
 out->evidence_state="SUPPORTED"; out->provenance_ref="research/BIUPIU-BENCHMARK-PROGRAM-HARVEST-GATE-2026-09-22.md"; return 0;
}
