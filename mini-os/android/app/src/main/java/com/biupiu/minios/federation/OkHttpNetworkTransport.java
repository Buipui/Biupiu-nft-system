...
/** Single network boundary for external HTTP federation. */
public final class OkHttpNetworkTransport implements NetworkTransport, ExternalTransportAdapter {
    private final OkHttpClient client;
    public OkHttpNetworkTransport(OkHttpClient client) {
    ...
    @Override public NetworkResponse executeGet(String url, Map<String,String> headers) throws IOException {
        ...
        if (headers != null) for (Map.Entry<String,String> h : headers.entrySet()) if (h.getKey()!=null && h.getValue()!=null) request.header(h.getKey(),h.getValue());
        try (Response response = client.newCall(request.get().build()).execute()) {
            return new NetworkResponse(response.code(), response.body()==null ? "" : response.body().string());
        }
    }
